...
```{r ans13}
grade <- c(50,60,92,88)
allgrade <- fromEcon*grade
SN <- sum(fromEcon)
sum(SN)
sum(allgrade)
meanEcon <- sum(allgrade)/sum(SN)
meanEcon
```
...
```{r ans31}
earnedCredit1 <- grade >= 60
earnedCredit1 <- as.integer(earnedCredit1)
earnedCredit <- earnedCredit1*credit
...
```{r ans32}
credit100ChosenStudent <- sum(studentID == "046S6f645")
...
```{r ans33}
earnedCredit100ChosenStudent <- sum(studentID == "046S6f645" & grade >= 60)
