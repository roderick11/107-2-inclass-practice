...
commit8Date
twTime <- with_tz(commit8Date, tzone = "Asia/Taipei")
twTime
```
...
a <- c(1:30)
a <- as.character(a)
names(commitCheck) <- str_c("commit",a)
names(commitCheck)
commitCheck
```
...
# |代表或
```{r ans26}
logicWithinTimeFrame1 <- hour(allCommitDatesTW) == 4 & minute(allCommitDatesTW) >= 30 & minute(allCommitDatesTW) <= 59
logicWithinTimeFrame2 <- hour(allCommitDatesTW) == 5 
logicWithinTimeFrame3 <- hour(allCommitDatesTW) == 6 & minute(allCommitDatesTW) >= 0 & minute(allCommitDatesTW) <= 29
logicWithinTimeFrame <- logicWithinTimeFrame1 | logicWithinTimeFrame2 | logicWithinTimeFrame3
class(logicWithinTimeFrame)
...
```{r ans32}
logicFromEcon <- transcriptData[[3]] == "經濟學系"
logicFromEcon
```
